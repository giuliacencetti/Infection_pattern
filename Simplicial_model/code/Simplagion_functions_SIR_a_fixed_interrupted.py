...
class Simplagion_model:
    ...
    def simulate(self, beta, betaT, mu, save_temp_ev, t_i, lim_inf, lim_sup):
        '''
        Run the simulation.
...
#        flag_nb_inf = False
        ...
        for t in range(tmax):
            ...
            # 0- Ho raggiunto il range di attack rate richiesto
            ...
            if lim_inf < a < lim_sup:
                #print('nb infected + recovered',sum(self.Istate) + sum(self.Rstate))
                break
...
#        elif self.final_result == 'sing_traj':
#            mean_deg_t[t+1] = np.mean([self.degrees[n] for n in list(np.where(self.Istate==1)[0])])
#            return mean_deg_t[0:t+2]
