...
class Simplagion_model:
    ...
    def __init__(self, edgelist, edgelist3, nodes_list, final_result):
        '''
        Constructor.
            ...
            for i in old_inf:
                ...
                for n in range(len(indexes)):
                    ...
                    if beta*w < 0 or beta*w > 1:
                        print('errore p=',beta*w)
                    if r < beta*w:
                        self.Istate[j] = 1
                        if self.final_result == 'R0':
                            if i == self.pat0:
                                self.R0 += 1
            # TRIANGLES CONTAGION
            for triangle in self.edgelist3:
                n1, n2, n3 = triangle
                w = self.edgelist3[triangle]
                if betaT*w < 0 or betaT*w > 1:
                    print('errore p=',betaT*w)
                if n1 in old_inf:
                    if n2 in old_inf and n3 in old_susc:
                        r = np.random.uniform(0, 1)
                        if r < betaT*w:
                            self.Istate[n3] = 1
                            if self.final_result == 'R0':
                                if n1 == self.pat0 or n2 == self.pat0:
                                    self.R0 += 1
                    if n3 in old_inf and n2 in old_susc:
                        r = np.random.uniform(0, 1)
                        if r < betaT*w:
                            self.Istate[n2] = 1
                            if self.final_result == 'R0':
                                if n1 == self.pat0 or n3 == self.pat0:
                                    self.R0 += 1
                elif n1 in old_susc:
                    if n2 in old_inf and n3 in old_inf:
                        r = np.random.uniform(0, 1)
                        if r < betaT*w:
                            self.Istate[n1] = 1
                            if self.final_result == 'R0':
                                if n2 == self.pat0 or n3 == self.pat0:
                                    self.R0 += 1
